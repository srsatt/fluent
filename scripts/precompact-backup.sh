...
if compgen -G "$DATA_DIR"/*.sqlite > /dev/null; then
  find "$DATA_DIR" -maxdepth 1 -name "*.sqlite*" -exec cp {} "$BACKUP_DIR/" \;
fi

if compgen -G "$DATA_DIR"/*.json > /dev/null || compgen -G "$DATA_DIR"/*.sqlite > /dev/null; then
