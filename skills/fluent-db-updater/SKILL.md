...
description: Atomically update all Fluent learner stores at session end by calling scripts/update-db.py with a single JSON payload. Use at the end of every practice session — fluent-writing, fluent-vocab, fluent-speaking, fluent-reading, fluent-review, fluent-learn — to persist the session's errors, review results, new vocabulary, and session metadata.
---
...
Every practice skill ends with a DB update. Instead of hand-editing learner stores (error-prone, racy, easy to desync), pipe one JSON report to `update-db.py`. The script runs pre-write backups, validates the payload, applies all changes atomically, and rebuilds the spaced-repetition queue.
...
Returns all learner stores plus computed fields (`due_reviews_count`, `next_session_id`, `streak_active`, `days_since_last_session`).
...
The learner stores must agree: a new session-log entry, a bumped total session count, updated SM-2 params, new mistake patterns, recalculated accuracy, and refreshed streak. Hand-editing causes silent desync — streak says 7 days but session-log has 6 entries, mastery says 4 stars but accuracy says 45%. The helper script is the single write boundary.
