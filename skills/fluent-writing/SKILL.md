...
From `mastery-db.skills_mastery` and the learner's goals:

- Formal message (if register control needs work)
- Informal message (if everyday writing needs work)
- Form filling (if practical admin tasks are a goal)
- Short narrative or personal text (if overall writing < 3)
- Opinion, complaint, inquiry, or summary (B1+)
- Mixed scenarios (if all core forms are strong)

Scenarios must match the learner's CEFR level and adult context. A1-A2 uses concrete everyday situations; B1+ adds opinion, complaint, inquiry, summary, and genre control.
...
### Example 1 — formal message task (A2)

> ## ✍️ Writing Exercise
>
> **Scenario:** You need to cancel a doctor's appointment you have on Thursday.
>
> **Task:** Write a formal email to the doctor's office in the target language.
>
> **Requirements:**
> - Length: 40-60 words
...
Create a short language-specific reference only after reading the learner profile. Keep it focused on the current task: register, genre conventions, word order, task-required phrases, and common learner-language transfer issues.
