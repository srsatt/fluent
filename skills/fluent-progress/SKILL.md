...
This returns a single JSON object with all learner stores plus computed fields (`due_reviews_count`, `next_session_id`, `streak_active`, `days_since_last_session`).

If the helper is unavailable, fall back to reading each file directly. Resolve the data directory via `fluent_paths.data_dir()` first — do NOT hardcode `data/`:
...
> # 📊 Mohammad's Target-Language Learning Dashboard
>
...
Learner: "how am I doing with my target language?" → auto-invoke this skill, render the full dashboard.
...
- **Use target-language greetings** where natural.

## Why This Skill Auto-Invokes

Unlike session commands, this is a pure read. A false-positive dashboard costs only a few tokens — no DB corruption, no interrupted practice. Worth the lower trigger bar.
